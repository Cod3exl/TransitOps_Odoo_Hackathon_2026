...
function MaintenancePage() {
  ...
  const { data: services = [], isLoading: sLoading, isError } = useMaintenance();
  const addMaintMutation = useAddMaintenance();
  const closeMaintMutation = useCloseMaintenance();
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    addMaintMutation.mutate({
      ...
      serviceDate: new Date(form.date).toISOString(),
    }, {
      onSuccess: () => setForm({ vehicleId: "", serviceType: "", cost: "", date: today }),
    });
  };

  if (vLoading || sLoading) return <PendingComponent />;
  if (isError) return <div className="text-destructive p-4">Failed to load maintenance records.</div>;

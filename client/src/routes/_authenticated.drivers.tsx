...
function DriversPage() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    addDriverMutation.mutate({
      ...
      licenseExpiry: form.licenseExpiry ? new Date(form.licenseExpiry).toISOString() : new Date("2028-01-01").toISOString(),
      contactNumber: form.contact.trim() || undefined,
      safetyScore: 100,
    }, {
      onSuccess: () => {
        setDrawerOpen(false);
  ...
  if (isError) return <div className="text-destructive p-4">Failed to load drivers.</div>;
